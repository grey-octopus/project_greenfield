...
const ImageGallery = props => {
  ...
  if (
    props.styles[props.selected].photos[0] !== props.queue?.queue[0] &&
    props.queue &&
    props.queue?.queue[0] !== null
  ) {
    const photos = props.styles[props.selected].photos;
  ...
  if (props.styles && props.selected !== undefined && props.queue) {
    ...
    const inlineStyle = {
      backgroundImage: `url(${
        props.styles[props.selected].photos[props.selectedImage].url
      })`
    };
    const downArrow =
      photos.length > 7 &&
      props.queue.queue[props.queue.top - 2]?.url !==
    ...
    const rightArrow =
      props.selectedImage === photos.length - 1 ? null : ( // end of photos
        <i
    ...
    let id =
      props.isExpanded === false ? 'image-gallery' : 'image-gallery-expanded';
